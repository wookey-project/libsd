...
// FIXME: to be replaced by proper sdio API call
#define SDIO_CMDREND (1<<6)
#define SDIO_CMDSENT (1<<7)
#define SDIO_DATAEND (1<<8)
#define SDIO_DBCKEND (1<<10)
...
static void get_card_status(void)
{
    ...
    cmd.response = SHORT_RESP;
    //command_in_progress = SD_SEND_STATUS;
...
#if 0
static void sd_error(enum sd_error error)
{
    switch (error) {
    case SD_CMD_CRC_FAIL:
        printf("Command CRC fail (CMD%d)\n", current.cmd_value);
        break;
    case SD_DATA_CRC_FAIL:
        printf("Data CRC fail (CMD%d)\n", current.cmd_value);
        break;
    case SD_CMD_TIMEOUT:
        printf("Command timeout (CMD%d)\n", current.cmd_value);
        break;
    case SD_DATA_TIMEOUT:
        g_sd_card.timeout = 1;
        printf("Data timeout (CMD%d)\n", current.cmd_value);
        break;
    default:
        printf("Unknown error (CMD%d)\n", current.cmd_value);
    }
    dbg_flush();
    sd_error_retries--;
    if (sd_error_retries) {
        ...
        printf("Command error after %d retries\n", SD_ERROR_MAX_RETRIES);
    }
}
...
void sd_launch_dma(int i)
{
    ...
    if (i == 1) {               /*IN */
        //ret=fastcall(FAST_DMA_RECONF, (void*)&dma, DMA_RECONF_BUFIN | DMA_RECONF_BUFOUT | DMA_RECONF_BUFSIZE | DMA_RECONF_DIR);
...
//CF: Page 35
//  printf("sd_data_transfer_automaton lastcom %x\n",lastcom);
...
//  printf("flags %x\n",sd_getflags(0xffffffff));
            ...
            break;
        case SD_RCV:
            if ((lastcom == 25) && sdio_getflags(SDIO_DATAEND)) {
                send_cmd12();
            } else if (((lastcom == 12) && sdio_getflags(SDIO_CMDREND)) ||
                       ((lastcom == 24) && sdio_getflags(SDIO_DATAEND))) {
                g_sd_card.state = SD_PRG;
                nevents++;
            } else {
        ...
        case SD_DATA:
            ...
            if ((lastcom == 18) && sdio_getflags(SDIO_DATAEND)) {
                send_cmd12();   //STOP_TRANSMISSION
            } else if (((lastcom == 12) && sdio_getflags(SDIO_CMDREND)) ||
                       ((lastcom == 17) && sdio_getflags(SDIO_DATAEND))) {
            ...
            break;
        case SD_PRG:
            if ((lastcom == 12) || sdio_getflags(SDIO_DBCKEND)) //DataBlock End
            {
                g_sd_card.state = SD_TRAN;
            } else if (lastcom == 7) {
...
static void prepare_transfer(dma_dir_t dir, uint32_t * buffer, uint32_t buf_len)
{
    ...
    if (dir == PERIPHERAL_TO_MEMORY) {
        ...
        //ret = sys_cfg(CFG_DMA_RECONF, (void*)&dma_in, DMA_RECONF_BUFIN | DMA_RECONF_BUFSIZE); - FIXME
    } else {
        dma.in_addr = (physaddr_t) buffer;
        dma.out_addr = (volatile physaddr_t)sdio_get_data_addr();
        dma.size = buf_len;
        dma.dir = MEMORY_TO_PERIPHERAL;
        //ret = sys_cfg(CFG_DMA_RECONF, (void*)&dma_out, DMA_RECONF_BUFOUT | DMA_RECONF_BUFSIZE); - FIXME
    }
    if (ret != 0) {
        goto err;
    }
